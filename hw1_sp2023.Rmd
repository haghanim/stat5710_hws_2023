...
wsci <- read_excel("/Users/kevinyu/Desktop/Spring 2023/STAT 571/HW/Homework 1/data/WomenData_06_16.xlsx")
names(wsci)
head(wsci)
#change names
wsci %<>% 
...
```{r}
mlp <- read.csv("/Users/kevinyu/Desktop/Spring 2023/STAT 571/HW/Homework 1/data/MLPayData_Total.csv")
bb <- read.csv("/Users/kevinyu/Desktop/Spring 2023/STAT 571/HW/Homework 1/data/baseball.csv")
```
...
```{r}
rcorr(data_long$win_pct, data_long$diff_log)
```
...
data_agg %>%
  ...
  geom_smooth(method = "lm", formula = y ~ x, se = F, color = "yellow") + 
  geom_hline(aes(yintercept = mean(avg_win_pct)), color = "blue") +
  labs(title = "MLB Team's Overall Win  vs. Payroll_log", 
       x = "Diff_log_total", 
       y = "Win_pct_avg")
```
...
```{r, diff_log and win_pct by year}
data_long %>%
  filter(!is.na(diff_log)) %>%
...
```{r}
yearly_paroll <- lm(win_pct~payroll, data = bb)
summary(yearly_paroll)
```

```{r}
yearly_paroll <- lm(win_pct~diff_log, data = data_long)
summary(yearly_paroll)
```
