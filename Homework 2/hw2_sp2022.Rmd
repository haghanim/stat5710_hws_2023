...
```{r}
table(c(brca_subtype))
```
**There are four sub-types and in each subtype there are this amount of patients: Basal = 208, Her2 = 91, LumA = 628,  LumB = 233**
...
```{r}
len <- nrow(brca)
random_indices <- sample(len, 5)

five_r_patients <- data.frame(brca)[random_indices, ]

vector <- as.factor(as.factor(brca_subtype[random_indices]))

hist(table(vector), xlab = "subtypes", main="Histogram by each sub-type for 5 random genes", labels = levels(vector))
...
```{r}
brca <- brca[,-1]
non_zero_cols <- which(colSums(abs(brca)) != 0)
filtered_brca <- brca[, non_zero_cols, with=F]

transformed_brca <- log2(as.matrix(filtered_brca + 1e-10))
```
...
```{r}
num_centers <- 4
brca_kmeans <- kmeans(transformed_brca, num_centers)

table(brca_subtype, brca_kmeans$cluster)
```
...
**We should apply 4 PCs because the explained variance for each PC int he PCA result drops off after four.**

```{r}
scaled_centered_brca <- scale(transformed_brca) 
num_genes <- ncol(scaled_centered_brca)

pca_result <- irlba::irlba(scaled_centered_brca, 10, verbose = F)

n <- nrow(scaled_centered_brca)
explained_var <- (pca_result$d^2)/(n - 1)
explained_var_apx <- explained_var / num_genes
plot(explained_var_apx)

num_pcs <- 4
pca_result <- irlba::irlba(scaled_centered_brca, nv = num_pcs, verbose = F)
```
...
```{r}
brca_unscaled <- scale(transformed_brca, scale = F)

pca_result_unscaled <- irlba::irlba(brca_unscaled, num_pcs)

pc_unscaled_scores <- pca_result_unscaled$u[, 1:num_pcs] * pca_result_unscaled$d[1:num_pcs]

pc_scaled_scores <- pca_result$u[, 1:num_pcs] * pca_result$d[1:num_pcs]
```
```{r}
plot_unscaled <- data.table(x = pc_unscaled_scores[, 1], y = pc_unscaled_scores[, 2]) %>% 
  ggplot() + 
  geom_point(aes(x, y, col = as.factor(brca_subtype))) + 
  xlab("PC1") + 
  ylab("PC2") + 
...
plot_scaled <- data.table(x = pc_scaled_scores[, 1], y = pc_scaled_scores[, 2]) %>% 
  ggplot() + 
  geom_point(aes(x, y, col = as.factor(brca_subtype))) + 
  xlab("PC1") + 
  ylab("PC2") + 
...
ggpubr::ggarrange(plot_scaled, plot_unscaled)
```
**We should not scale because the unscaled data is clustered better for PC1 and PC2. Separation of categories is much better defined for unscaled brca than for scaled, so we should keep the data centered and unscaled.**
...
```{r}
calculate_wss <- function(data, num_clusters) {
  kmeans_result <- kmeans(data, num_clusters, nstart = 10)
  return (kmeans_result$tot.withinss)
}

number_of_clusters <- 1:10
wss_results <- sapply(number_of_clusters, function(k) calculate_wss(pc_unscaled_scores[, 1:4], k))
wss_data <- data.frame(k = number_of_clusters, wss = wss_results)

ggplot(data = wss_data, aes(x = k, y = wss)) +
  geom_point() +
  geom_line() +
  labs(x = "# of Clusters (K)", y = "WSS") +
  ggtitle("Within-Cluster Sum of Squares (WSS) vs Number of Clusters (K)")

```

**By the elbow rule, four clusters is an appropriate amount of clusters. **
...
kmeans_result <- kmeans(x = pc_unscaled_scores[, 1:num_pcs], centers = num_clusters)

cluster_centers <- data.frame(kmeans_result$centers[, 1:2])
cluster_centers$cluster <- as.factor(1:num_clusters)

plot_data <- data.frame(x = pc_unscaled_scores[, 1], 
                        y = pc_unscaled_scores[, 2],
                        label = as.factor(brca_subtype),
                        cluster = as.factor(kmeans_result$cluster))

PC1 <- pc_unscaled_scores[, 1]
PC2 <- pc_unscaled_scores[, 2]
...
ggplot(data = plot_data) + 
  geom_point(aes(x, y, color = label, shape = cluster)) + 
  geom_point(data = cluster_centers, aes(x = cluster_centers[, 2], y = cluster_centers[, 2], shape = cluster), size = 5) +
  
  labs(title = "Centered and Unscaled PC1 vs PC2 (k = 4)", x = "PC 1", y = "PC 2") +
  theme_classic()
```
...
```{r}
tab1 <- table(brca_subtype, brca_kmeans$cluster)
tab2 <- table(brca_subtype, kmeans_result$cluster)

tab1
tab2
```
**Yes, significantly. Using PCA minimized the sum of squares within each cluster effectively as it significantly reduces the total within cluster sum of squares compared to the original dataset. We can see this as there are larger numbers of cancer types into each subtype.**
...
**X_patient belongs in cluster two which most closely resembles LumA**

```{r}
x_patient <- fread("data/brca_x_patient.csv")

x_patient <- x_patient[ , which(colnames(x_patient) %in% colnames(transformed_brca))]

x_patient <- log2(as.matrix(x_patient+1e-10))

x_patient_c <- scale(x_patient, scale = F)
x_patient_pcscores <- as.vector(x_patient_c) %*% pca_result_unscaled$v[,1:4]

x_patient_d <- kmeans_result$centers - t(matrix((x_patient_pcscores), nrow=4, ncol=4))
x_patient_d <- sqrt(rowSums(x_patient_d ^ 2))

x_patient_d
which.min(x_patient_d)

ggplot(data = plot_data) + 
  geom_point(aes(x, y, color = label, shape = cluster)) + 
  geom_point(data = cluster_centers, aes(x = cluster_centers[, 2], y = cluster_centers[, 2], shape = cluster), size = 5) +
  geom_point(aes(x = x_patient_pcscores[1], y = x_patient_pcscores[2]))
  labs(title = "Centered and Unscaled PC1 vs PC2 (k = 4)", x = "PC 1", y = "PC 2") +
  theme_classic()
```
